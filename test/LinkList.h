...
//打印
extern linkStatus linkListVisit (linkElemType c);					//打印元素
extern linkStatus linkListPrintTraverse (LinkList L);			//打印顺序
...
extern linkStatus linkListClear (LinkList* L);				//清空（free）
extern linkStatus linkListClearManu (LinkList* L);			//清空
extern linkStatus linkListEmpty (LinkList L);				//空？
extern int16_t linkListLength (LinkList L);					//长度
extern linkStatus linkListGetElem (LinkList L, int i, linkElemType* e);						//得到索引值
...
extern void linkListDelxRecursion (LinkList* L, int x);//删除x的递归
